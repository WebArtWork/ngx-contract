...
export class ContractService {
	...
	new_contract(users, contract, title) {
		this.modal.show({ component: CreateComponent, users, contract, title });
	}
	...
	constructor(private mongo: MongoService, private alert: AlertService, public modal: ModalService) {
		this.contracts = mongo.get('contract', {
			// groups: {
			// 	place: {
			// 		field: doc => { return doc.place },
			// 		allow: doc => doc.status === 'Accepted'
			// 	}
			// }
		}, (arr, obj) => {
			this._contracts = obj;
